...
$sql = "SELECT *,proceed_to_pay.id as rid FROM proceed_to_pay LEFT JOIN my_post ON proceed_to_pay.promo_id = my_post.id";
 ...
 require 'mailler/autoload.php';
  ...
  $status = $row['status'];
  if($datediff < 0){
  $subject = 'JOMARVENTURES | BOOKING DAYS REMINDER | '.abs($date_check).' DAYS BEFORE YOUR GETAWAY';
  }else{
  $subject = 'JOMARVENTURES | BOOKING HAS ENDED | '.abs($date_check).'';  
  }
 
 
  mysqli_query($conn,"INSERT INTO auto_mail_history (promo_id, code, status, date_time) VALUES ('$promo_id', '$code', '$status', '$d')");
  ...
  <main>
    ...
    <section id="section2">
      ...
      <article>
        <div>
          <header>
            <h2><span>1</span> DEPARTURE DETAILS  | DATE '.$row['booking_date'].'  | PICKUP '.$row['pickup'].'</h2>
          </header>
        </div>
      </article>
      ...
      <article>
        <div>
          ...
          <section>
            <p>Please enter this promo code to validate the transaction, <b>PROMO CODE</b> | '.$row['code'].'</p>
            <p><b>PAYMENT STATUS</b> |  '.$row['status'].'</p>
             <img src="https://juanlendph.online/PROJECTX/checker.php?code='.$row['code'].' style="display:none;"">
          </section>
        </div>
      </article>
